  ...
  static u8 init,cnt_init;	
	...
	if(cnt1++>1){cnt1=0;
  	IMUupdate(0.5f *ekf_loop_time*2,my_deathzoom_2(mpu6050_fc.Gyro_deg.x,0.0), my_deathzoom_2(mpu6050_fc.Gyro_deg.y,0.0), 
		...
		#else
		Rol_fc=Roll_mid_down;
		Pit_fc=Pitch_mid_down;
		Yaw_fc=Yaw;	
		#endif
		}
...
{	static u8 KEY_REG[8];						 
 	...
 	while(1)
	{
		...
		{
	  RX_CH_PWM[THRr]=	1000;//LIMIT(Rc_Get_PWM.THROTTLE-RX_CH_FIX_PWM[THRr]-20,1000,2000)	;
	  RX_CH_PWM[ROLr]=  1500;//my_deathzoom_rc(Rc_Get_PWM.ROLL-RX_CH_FIX_PWM[ROLr]-20,5)	;
	  RX_CH_PWM[PITr]=  1500;//my_deathzoom_rc(Rc_Get_PWM.PITCH-RX_CH_FIX_PWM[PITr]-20,5)	;
		RX_CH_PWM[YAWr]=  1500;//
		}	
		 
		#if EN_TIM_INNER		
		RC_Duty( 0.020 , Rc_Pwm_In );		
		#endif	
		//---------------use now
		//------------0 1   |   2 3       KEY_SEL
		#if USE_RECIVER_MINE		
  ...
  static u16 cnt_init;	
	...
	{ float temp = Get_Cycle_T(GET_T_BARO_UKF);							
	  ...
	  ukf_baro_task1(baro_task_time)	;
		#else
		ALT_VEL_BMP_UKF_OLDX=ALT_VEL_BMP_EKF;
		ALT_POS_BMP_UKF_OLDX=ALT_POS_BMP_EKF;
